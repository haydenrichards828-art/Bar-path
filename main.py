...
app = FastAPI(title="ForceTrack Bar Path API", version="0.8.0")
...
GF_PARAMS = dict(maxCorners=150, qualityLevel=0.01, minDistance=3, blockSize=7)
LK_REFRESH       = 15
TEMPLATE_REFRESH = 60
FB_THRESHOLD     = 1.5
...
def lk_track_bidirectional(prev_gray, curr_gray, pts):
    if pts is None or len(pts) < 4: return None, None, None
    new_pts, st_f, _ = cv2.calcOpticalFlowPyrLK(prev_gray, curr_gray, pts, None, **LK_PARAMS)
    if new_pts is None: return None, None, None
    back_pts, st_b, _ = cv2.calcOpticalFlowPyrLK(curr_gray, prev_gray, new_pts, None, **LK_PARAMS)
    if back_pts is None: return None, None, None
    fb_err = np.sqrt(((pts - back_pts) ** 2).sum(axis=2)).ravel()
    good_mask = (st_f.ravel() == 1) & (st_b.ravel() == 1) & (fb_err < FB_THRESHOLD)
    good_new = new_pts[good_mask]
...
async def analyze(video: UploadFile=File(...), params: str=Form("{}"), api_key: str=Form("")):
    ...
    try:
        ...
        def make_template(gray, cx, cy):
            pad = int(plate_r * 1.8)
            x1, y1 = max(0, int(cx-pad)), max(0, int(cy-pad))
            x2, y2 = min(wp, int(cx+pad)), min(hp, int(cy+pad))
            return gray[y1:y2, x1:x2].copy()

        plate_tmpl = make_template(g0, cx0, cy0)
        ...
        while True:
            t = actual_ts[frame_idx] if (actual_ts and frame_idx < len(actual_ts)) else frame_idx / fps
            ret, frame = cap.read()
            if not ret: break
            ...
            if frame_idx > 0 and frame_idx % TEMPLATE_REFRESH == 0 and consecutive_bad == 0:
                plate_tmpl = make_template(gray, last_cx, last_cy)

            lk_cx, lk_cy, lk_pts_new = lk_track_bidirectional(prev_gray, gray, lk_pts)
            ok, bbox = tracker.update(frame)

            def _reinit(rx, ry):
                nonlocal tracker, last_cx, last_cy, lk_pts, consecutive_bad, plate_tmpl
                ...
                plate_tmpl = make_template(gray, rx, ry)
            ...
            if lk_cx is not None:
                ...
                if lk_jump > max_jump_sq:
                    ...
                    consecutive_bad = 0
                    cx, cy = lk_cx, lk_cy  # pure LK, no CSRT blend
